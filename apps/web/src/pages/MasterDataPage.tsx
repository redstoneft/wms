    ...
    <div>
      ...
      <Modal open={!!token} onClose={() => setToken(null)} title={`Estación de impresión · ${token?.printer ?? ''}`}>
        ...
        <ol className="mt-3 list-decimal space-y-1 pl-5 text-sm text-slate-700">
          <li>En la PC de la Zebra descarga <span className="font-mono">estacion_wms.bat</span> y dale doble clic. Se copia solo a la carpeta Inicio de Windows y arranca la estación.</li>
          <li>Si la PC tiene Python instalado (python.org, "Add Python to PATH"), la estación queda corriendo <b>en segundo plano, sin ventana</b>: imprime por el driver normal de Windows y además atiende a la app de etiquetas SAE (127.0.0.1:9101), así las dos apps comparten la Zebra. Su registro queda en <span className="font-mono">%LOCALAPPDATA%\wms-print\estacion.log</span> y se detiene con <span className="font-mono">detener_estacion.bat</span> de esa misma carpeta. Si no hay Python, abre la estación del navegador: ahí presionas <b>Elegir la Zebra</b> la primera vez.</li>
        ...
        <div className="mt-3 flex flex-wrap gap-2">
          <a className="rounded-md bg-emerald-600 px-4 py-2 text-sm font-semibold text-white" href={URL.createObjectURL(new Blob([`@echo off\r\nrem Estacion de impresion WMS - ${token?.printer ?? ''}. Doble clic una vez: queda corriendo en segundo plano (sin ventana) y arranca sola al prender la PC.\r\nrem Con Python instalado imprime por el driver de Windows y atiende tambien a la app de etiquetas SAE (127.0.0.1:9101). Sin Python abre la estacion del navegador.\r\ntitle Estacion de impresion WMS - ${token?.printer ?? ''}\r\nset "WMS_URL=${window.location.origin}"\r\nset "WMS_PRINT_TOKEN=${token?.token ?? ''}"\r\nset "URL=${window.location.origin}/print-station#token=${token?.token ?? ''}"\r\nset "INICIO=%APPDATA%\\Microsoft\\Windows\\Start Menu\\Programs\\Startup\\"\r\nset "DIR=%LOCALAPPDATA%\\wms-print\\"\r\nif not exist "%DIR%" mkdir "%DIR%"\r\npython --version >nul 2>nul\r\nif errorlevel 1 goto navegador\r\necho Descargando la estacion...\r\npowershell -NoProfile -ExecutionPolicy Bypass -Command "try { [Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12; Invoke-WebRequest -UseBasicParsing '%WMS_URL%/print-agent/wms_print_agent.py' -OutFile '%DIR%wms_print_agent.py' } catch {}"\r\nif not exist "%DIR%wms_print_agent.py" goto navegador\r\npython -m pip install --quiet pywin32 requests >nul 2>nul\r\nrem --- bucle que corre sin ventana (pythonw) y se reinicia solo ---\r\n> "%DIR%estacion_loop.bat" (\r\n  echo @echo off\r\n  echo set "WMS_URL=%WMS_URL%"\r\n  echo set "WMS_PRINT_TOKEN=%WMS_PRINT_TOKEN%"\r\n  echo cd /d "%DIR%"\r\n  echo :loop\r\n  echo pythonw wms_print_agent.py\r\n  echo if errorlevel 3 exit\r\n  echo timeout /t 5 ^>nul\r\n  echo goto loop\r\n)\r\n> "%DIR%estacion_oculta.vbs" echo CreateObject("WScript.Shell").Run "cmd /c """"%DIR%estacion_loop.bat""""", 0, False\r\n> "%DIR%detener_estacion.bat" (\r\n  echo @echo off\r\n  echo taskkill /f /im pythonw.exe ^>nul 2^>nul\r\n  echo echo Estacion de impresion WMS detenida. Para volver a iniciarla: doble clic en estacion_wms.bat\r\n  echo timeout /t 5\r\n)\r\n> "%INICIO%estacion_wms.bat" (\r\n  echo @echo off\r\n  echo timeout /t 20 ^>nul\r\n  echo wscript "%DIR%estacion_oculta.vbs"\r\n)\r\ntaskkill /f /im pythonw.exe >nul 2>nul\r\nwscript "%DIR%estacion_oculta.vbs"\r\necho.\r\necho La estacion quedo corriendo en segundo plano, sin ventana, y arrancara sola al prender la PC.\r\necho Comprueba en el WMS: Datos maestros - Impresoras - "conectada".\r\necho Registro: %DIR%estacion.log   Para detenerla: %DIR%detener_estacion.bat\r\ntimeout /t 12\r\nexit /b 0\r\n:navegador\r\nif /i not "%~dp0"=="%INICIO%" copy /y "%~f0" "%INICIO%estacion_wms.bat" >nul 2>nul\r\nif /i "%~dp0"=="%INICIO%" timeout /t 20 >nul\r\nstart "" msedge --app="%URL%" 2>nul || start "" chrome --app="%URL%" 2>nul || start "" "%URL%"\r\n`], { type: 'application/octet-stream' }))} download="estacion_wms.bat" data-testid="agent-download-station">
        ...
        <details className="mt-3 text-xs text-slate-500">
          <summary>Reparar la Zebra / usarla junto con la app de etiquetas SAE · y opción B (estación Python)</summary>
          <p className="mt-2"><b>reparar_impresora.bat</b> (pide administrador) regresa la Zebra al driver normal de Windows si quedó en WinUSB por WebUSB, limpia la cola, crea la impresora "ZEBRA" e imprime una prueba. Después de eso la app de etiquetas SAE (EstacionZebra.exe) y la estación del WMS imprimen las dos por Windows sin pelearse.</p>
