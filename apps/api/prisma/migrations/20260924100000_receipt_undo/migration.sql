-- Receiving corrections: a scan registered by mistake is reversed with RECEIPT_UNDO (FROM side only, like SCRAP).
-- The trigger below is the latest version (20260909110000_mixed_pallet_move) plus the new type.
...
ALTER TABLE inventory_movements ADD CONSTRAINT ck_mov_type CHECK (movement_type IN
    ('RECEIPT','PUTAWAY','TRANSFER_START','TRANSFER_COMPLETE','TRANSFER_CANCEL','REPLENISH_START','REPLENISH_COMPLETE',
     ...
     'RETURN_RECEIPT','SCRAP','LPN_SPLIT','LPN_CONSOLIDATE','INITIAL_LOAD','ASSEMBLY_OUT','ASSEMBLY_IN','RECEIPT_UNDO'));
...
DECLARE
  ...
  outbound boolean := NEW.movement_type IN ('SHIP','ADJUST_OUT','COUNT_ADJUST_OUT','SCRAP','ASSEMBLY_OUT','RECEIPT_UNDO');
  v_lpn_status text;
  v_from_loc uuid;
  v_sibling boolean;
BEGIN
