...
sub render(data as object)
  ...
  for each it in items
    if it.status = "PLANNED" and it.delivery_date < today
  ...
  ' the calendar: from today onwards, 12 working days (Mon..Sat) in two rows; Sunday deliveries show in Saturday's box
  days = []
  off = 0
  while days.Count() < 12 and off < 20
    d = localDate(off)
    if d.GetDayOfWeek() <> 0
      iso = isoOf(d)
      cellItems = []
      ...
      if d.GetDayOfWeek() = 6
        sundayIso = isoOf(localDate(off + 1))
        for each it in items
      ...
      days.Push({ d: d, iso: iso, list: cellItems })
    end if
    off = off + 1
  end while
  ' each row takes height in proportion to its busiest day, so a loaded day is not cut off
  ncols = 6
  gap = 8
  ...
  for i = 0 to days.Count() - 1
    r = Int(i / ncols)
    n = days[i].list.Count()
    if n > load[r] then load[r] = n
  end for
  rowH0 = Int(totalH * load[0] / (load[0] + load[1]))
  if rowH0 < 200 then rowH0 = 200
  if rowH0 > totalH - 200 then rowH0 = totalH - 200
  rowH = [rowH0, totalH - rowH0]
  for i = 0 to days.Count() - 1
    r = Int(i / ncols)
    c = i mod ncols
    x = c * (colW + gap)
    y = 0
    if r = 1 then y = rowH[0] + gap
    day = days[i]
    m.grid.appendChild(cellNode(x, y, colW, rowH[r], day.d, day.iso = today, day.list))
  end for
end sub

function cellNode(x as integer, y as integer, w as integer, h as integer, d as object, isToday as boolean, cellItems as object) as object
  g = CreateObject("roSGNode", "Group")
  g.translation = [x, y]
  bg = CreateObject("roSGNode", "Rectangle")
  bg.width = w
  bg.height = h
  if isToday then bg.color = "0x3B2F0BFF" else bg.color = "0x1E293BFF"
  g.appendChild(bg)
  if isToday
  ...
  if isToday
    ...
    head.color = "0xE2E8F0FF"
    head.text = dayName(d.GetDayOfWeek()) + " " + Str(d.GetDayOfMonth()).Trim() + " " + monthName(d.GetMonth())
  end if
  g.appendChild(head)
  ' type shrinks with the number of deliveries so every one fits in the cell
  n = cellItems.Count()
  if n <= 3
    rowFont = "font:MediumBoldSystemFont"
    noteFont = "font:SmallSystemFont"
    spacing = 8
    noteLines = 3
  else if n <= 6
    rowFont = "font:SmallBoldSystemFont"
    noteFont = "font:SmallestSystemFont"
    spacing = 4
    noteLines = 2
  else
    rowFont = "font:SmallestBoldSystemFont"
    noteFont = "font:SmallestSystemFont"
    spacing = 2
    noteLines = 1
  end if
  ...
  list.itemSpacings = [spacing]
  list.clippingRect = [0, 0, w - 24, h - 66]
