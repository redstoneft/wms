...
export async function startTransfer(tx: Tx, ctx: ActorContext, input: { lpn_code: string; to_location_barcode: string; reason?: string; transfer_type?: 'LOCATION' | 'REPLENISHMENT' }) {
  ...
  // pallets waiting in receiving/returns leave only through directed put-away (never by free transfer)
  const fromLoc = await tx.locations.findUnique({ where: { id: lpn.current_location_id }, select: { location_type: true, code: true } });
  if (fromLoc && ['RECEIVING', 'RETURNS'].includes(fromLoc.location_type)) {
    throw new RuleError('USE_PUTAWAY', `LPN ${lpn.code} is at ${fromLoc.code}; pallets leave receiving through directed put-away`);
  }
  const pendingPutaway = await tx.putaway_tasks.count({ where: { lpn_id: lpn.id, status: { in: ['PENDING', 'ASSIGNED', 'IN_PROGRESS'] } } });
  if (pendingPutaway) throw new RuleError('PUTAWAY_PENDING', `LPN ${lpn.code} has an active put-away task; complete or cancel it first`);
  ...
  // only "static" stock moves: everything on the pallet must share one of AVAILABLE/QUARANTINE/BLOCKED/DAMAGED
  const statuses = [...new Set(balances.map((b) => b.status))];
  const STATIC = ['AVAILABLE', 'QUARANTINE', 'BLOCKED', 'DAMAGED'];
  if (statuses.length !== 1 || !STATIC.includes(statuses[0]!)) {
    throw new RuleError('LPN_NOT_AVAILABLE', `LPN ${lpn.code} has inventory in ${statuses.join(', ')}; only a pallet whose whole content is AVAILABLE, QUARANTINE, BLOCKED or DAMAGED can be transferred`);
  }
  const originStatus = statuses[0] as 'AVAILABLE' | 'QUARANTINE' | 'BLOCKED' | 'DAMAGED';
  ...
  const transfer = await tx.transfers.create({
    data: { transfer_type: crossWarehouse ? 'WAREHOUSE' : (input.transfer_type ?? 'LOCATION'), lpn_id: lpn.id, from_location_id: lpn.current_location_id, to_location_id: to.id, started_by: ctx.userId, reason: input.reason ?? null, origin_status: originStatus },
  });
...
export async function completeTransfer(tx: Tx, ctx: ActorContext, input: { transfer_id: string; lpn_code: string; location_barcode: string }) {
  const rows = await tx.$queryRaw<{ id: string; lpn_id: string; to_location_id: string; from_location_id: string; status: string; transfer_type: string; origin_status: string }[]>`
    SELECT id, lpn_id, to_location_id, from_location_id, status, transfer_type, origin_status FROM transfers WHERE id = ${input.transfer_id}::uuid FOR UPDATE`;
  const t = rows[0];
  ...
  const toStatus = (scanned.location_type === 'QUARANTINE' ? 'QUARANTINE' : scanned.location_type === 'DAMAGED' ? 'DAMAGED' : t.origin_status) as 'AVAILABLE' | 'QUARANTINE' | 'BLOCKED' | 'DAMAGED';
...
export async function cancelTransfer(tx: Tx, ctx: ActorContext, transferId: string, reason: string) {
  const rows = await tx.$queryRaw<{ id: string; lpn_id: string; from_location_id: string; status: string; origin_status: string }[]>`
    SELECT id, lpn_id, from_location_id, status, origin_status FROM transfers WHERE id = ${transferId}::uuid FOR UPDATE`;
  const t = rows[0];
  ...
  for (const b of balances.filter((x) => x.status === 'IN_TRANSFER')) {
    await recordMovement(tx, ctx, {
      ...
      to_status: t.origin_status as 'AVAILABLE' | 'QUARANTINE' | 'BLOCKED' | 'DAMAGED',
      transfer_id: t.id,
      reason,
