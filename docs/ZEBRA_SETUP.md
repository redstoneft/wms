...
## Etiquetas sueltas
`Etiquetas → Etiquetas sueltas`: lista de códigos (o lo que escanea el lector, `LOC-…`) y un nombre; genera el pedido `.json` para la app de etiquetas (`WMS-<almacén>-<nombre>`), la hoja imprimible o el ZPL solo de esas ubicaciones. Sirve para las que faltaron de un rack ampliado, los puentes y las reimpresiones.
...
1. WMS → Datos maestros → Impresoras → la Zebra → **Generar token** → **Descargar estacion_wms.bat** (un solo archivo, ya trae el token). Doble clic: se copia solo a la carpeta Inicio de Windows y abre la estación en su propia ventana de Edge/Chrome.
