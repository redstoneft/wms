...
| Sincronización SAE | `test/integration/sae-sync.test.ts` | PostgREST simulado en memoria (filtros `eq/in/gte`, paginación por `Range`): un SKU por producto (GTIN/modelo) con todas las claves SAE como alias con su UoM, fusión auditada de SKUs por clave heredados (conservando códigos de barras), resolución de cualquier clave por `/skus/by-barcode`, desactivación sin inventario, clientes SAE + plataforma, proveedores, OC con líneas agregadas por producto y OC recibida intocable, pedidos cuyas líneas de distintas claves del mismo producto se consolidan, pedido rechazado completo si una línea no resuelve, cancelación → cancelado o incidencia, segunda corrida idempotente, comparación de existencias, fuentes sin configurar → error claro |
