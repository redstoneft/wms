...
| 39 | Integration layer desacoplada (SAE) | Entrada: `modules/integrations` (API key). **Sincronización real SAE → WMS**: `modules/sae` lee los espejos Supabase de SAE (artículos con modelo/capa, conversiones caja, GTIN, clientes, proveedores, órdenes de compra, pedidos retail) cada 30 min, idempotente, sin tocar inventario; comparación de existencias. Ver `docs/INTEGRATION_SAE.md` | net "integration layer", `sae-sync.test.ts` (PostgREST simulado) + corrida real (1,241 SKUs, 214 clientes, 38 proveedores, 16 OC, 4 pedidos) | ✅ |
