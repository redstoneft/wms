...
3. **Ver en TV**: un supervisor genera el **enlace del tablero** (una sola vez) y se abre en el navegador de la TV o de la computadora conectada a ella, en pantalla completa. El tablero se actualiza solo cada 30 segundos y no pide usuario: el enlace es de solo lectura y solo muestra el calendario. Si el enlace se filtra, se revoca y se genera otro.
5. **TV Roku**: no tiene navegador; se instala el tablero como app propia. En *Ver en TV* un supervisor descarga `tablero_roku.zip` (trae su enlace). En el Roku se activa el modo desarrollador (control: Inicio ×3, Arriba ×2, Derecha, Izquierda, Derecha, Izquierda, Derecha), se pone contraseña y se anota la IP; desde una PC de la misma red se abre `http://IP-del-Roku` (usuario `rokudev`), Upload → el zip → Install. La app "Tablero de entregas WMS" queda al final de los canales; se desactivan protector de pantalla y apagado automático y se deja abierta. Se actualiza sola cada 30 segundos.
4. **TV siempre encendida con el calendario**: lo recomendado es una PC o mini PC con Windows conectada por HDMI a la TV. En *Ver en TV* se descarga `tablero_tv.bat`; con un doble clic la PC deja de apagar la pantalla y de suspenderse, se copia a la carpeta Inicio y abre el tablero en pantalla completa (kiosco) cada vez que se enciende. En la TV se desactiva el apagado automático por inactividad. Alternativa: Android TV con la app *Fully Kiosk Browser* apuntando al enlace (con "mantener pantalla encendida" y arranque automático). El navegador de la propia TV no es recomendable: suele suspenderse. El tablero mantiene la pantalla despierta cuando el navegador lo permite, sigue mostrando la última información si se cae la red y se actualiza solo cuando hay una versión nueva del WMS.
...
* **Confirmar armado**, cuando terminen (ese día u otro, cualquier usuario): en la portada de Armado toca el armado abierto → por cada tarima captura **cajas completas**, las **piezas de la caja incompleta** si la hay (por ejemplo 59 cajas de 12 y una caja con 10) y las **defectuosas** que salieron en esa tarima, con su motivo. La pantalla muestra consumido, producido y defectuosas; si aún faltan piezas por explicar, el siguiente paso las registra como merma adicional con motivo → **Confirmar armado**. Ahí se consumen los cuerpos, nacen las tarimas nuevas con etiqueta y tarea de acomodo, y las defectuosas quedan como incidencia. Al ubicar las tarimas nuevas se elige la posición de la lista como en cualquier acomodo.
...
* **Cambiar el destino de una tarima ya armada y reimprimir**: en la portada de Armado, *Armados terminados* → toca el armado → en cada tarima **Cambiar ubicación** (lista de posiciones válidas, primero las que ya tienen el producto) y **Reimprimir con la nueva ubicación**. La etiqueta LPN imprime el destino (línea DESTINO y pie "→ DESTINO …") mientras la tarima tenga acomodo pendiente.
